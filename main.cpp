    ...
    switch (event)
    {
        case CV_EVENT_LBUTTONDOWN:
            std::cout << "at(" << x << "," << y << ")value is:"
                      << static_cast<int>(im->at<float>(cv::Point(x, y))) << std::endl;
            break;
    }
}
...
int main(int argc, char** argv)
{
    ...
    for (int p = 0;; p++)
    {
        ...
        setMouseCallback("srcvideo", onMouse, &depthmat);  //第三个参数void* 用户数据

        Mat dep,dep2;
        depthmat.copyTo(dep);
        depthmat.copyTo(dep2);
        threshold(dep,dep,797,4500,0);
        threshold(dep2,dep2,890,4500,0);
        dep.convertTo(dep, CV_8UC1, 255.0);
        dep2.convertTo(dep2, CV_8UC1, 255.0);
        imshow("depthmat",dep);
        imshow("depthmat2",dep2);
